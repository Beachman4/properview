...
export class PropertiesService {
  ...
  async paginate(
    ...
    ServerInferResponseBody<typeof contract.public.properties.list, 200>
  > {
    const where: Prisma.PropertyWhereInput = {
      status: PropertyStatus.active,
      ...(queryParams.bedroomsMin && {
        bedrooms: { gte: queryParams.bedroomsMin },
      }),
      ...(queryParams.bedroomsMax && {
        bedrooms: { lte: queryParams.bedroomsMax },
      }),
      ...(queryParams.bathroomsMin && {
        bathrooms: { gte: queryParams.bathroomsMin },
      }),
      ...(queryParams.bathroomsMax && {
        bathrooms: { lte: queryParams.bathroomsMax },
      }),
    ...
    if (queryParams.location) {
      const coordinates = await this.mapboxService.getCoordinates(
        queryParams.location,
      );
      const [longitude, latitude] = coordinates;

      // Build dynamic WHERE conditions using Prisma.sql
      const whereConditions = [Prisma.sql`p.status = 'active'`];

      if (queryParams.bedroomsMin) {
        whereConditions.push(
          Prisma.sql`p.bedrooms >= ${queryParams.bedroomsMin}`,
        );
      }
      if (queryParams.bedroomsMax) {
        whereConditions.push(
          Prisma.sql`p.bedrooms <= ${queryParams.bedroomsMax}`,
        );
      }
      if (queryParams.bathroomsMin) {
        whereConditions.push(
          Prisma.sql`p.bathrooms >= ${queryParams.bathroomsMin}`,
        );
      }
      if (queryParams.bathroomsMax) {
        whereConditions.push(
          Prisma.sql`p.bathrooms <= ${queryParams.bathroomsMax}`,
        );
      }
      ...
      const totalWithDistance = await this.prisma.$queryRaw<
        Array<{ total: bigint }>
      >(
        Prisma.sql`
    ...
    const orderBy: Prisma.PropertyOrderByWithRelationInput = {
      ...
      ...(queryParams.sortBy === 'bedrooms' && {
        bedrooms: queryParams.sortOrder,
      }),
  ...
  async paginateByAgentId(
    agentId: string,
    page: number = 1,
    limit: number = 10,
  ): Promise<
    ServerInferResponseBody<typeof contract.agent.properties.list, 200>
  > {
    const [properties, total] = await this.prisma.$transaction([
      this.prisma.property.findMany({
        where: {
    ...
    return {
      data: properties.map((property) =>
        this.hydrateProperty(property, property._count.inquiries),
      ),
      meta: {
        total,
        page,
  ...
  async create(
    agentId: string,
    data: ServerInferRequest<typeof contract.agent.properties.create>['body'],
  ) {
  ...
  async update(
    id: string,
    agentId: string,
    data: ServerInferRequest<typeof contract.agent.properties.update>['body'],
  ) {
