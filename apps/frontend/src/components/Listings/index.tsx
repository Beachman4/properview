...
export default function Listings() {
  ...
  const debouncedUpdateURL = useDebouncedCallback((newFilters: Filters, newViewMode: ViewMode) => {
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {bedroomOptions.map((option) => (
              <Button
                key={option.value}
                variant={String(filters.bedroomsMin || '') === option.value ? 'default' : 'outline'}
                size="sm"
                onClick={() => handleFilterChange('bedroomsMin', option.value ? Number(option.value) : undefined)}
              >
                {option.label}
              </Button>
            ))}
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {bathroomOptions.map((option) => (
              <Button
                key={option.value}
                variant={String(filters.bathroomsMin || '') === option.value ? 'default' : 'outline'}
                size="sm"
                onClick={() => handleFilterChange('bathroomsMin', option.value ? Number(option.value) : undefined)}
              >
                {option.label}
              </Button>
            ))}
...
function MapView({ properties, isLoading }: { properties: Property[], isLoading: boolean }) {
  ...
  useEffect(() => {
    ...
    const centerLat = properties.reduce((sum, p) => sum + p.addressLatitude, 0) / properties.length
    const centerLng = properties.reduce((sum, p) => sum + p.addressLongitude, 0) / properties.length
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-lg shadow-sm">
        <p className="text-sm font-medium">{properties.length} properties shown</p>
      </div>
    </div>
  )
}