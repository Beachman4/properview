            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium">
                    {(propertyData.conversionRate * 100).toFixed(2)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">
                  ...
                  <span className="text-sm font-medium">
                    {Math.floor(
                      (Date.now() -
                        new Date(propertyData.createdAt).getTime()) /
                        (1000 * 60 * 60 * 24),
                    )}{" "}
                    days
                  </span>
...
function InquiriesSection({ propertyId }: { propertyId: string }) {
  ...
  const { data: inquiries, isLoading: inquiriesLoading } =
    tsr.agent.inquiries.list.useQuery({
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        ...
        {hasInquiries && (
          <Badge variant="secondary">{inquiriesData.meta.total} total</Badge>
        )}
      </CardHeader>
      <CardContent>
        {!hasInquiries ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-muted-foreground">
              Inquiries will appear here when customers are interested in this
              property.
            </p>
          </div>
        ) : (
            ...
            {inquiriesData.meta.totalPages > 1 && (
              <div className="flex items-center justify-between pt-4 border-t">
                <div className="text-sm text-muted-foreground">
                  Page {inquiriesData.meta.page} of{" "}
                  {inquiriesData.meta.totalPages}
                  {" • "}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    {Array.from(
                      { length: Math.min(5, inquiriesData.meta.totalPages) },
                      (_, i) => {
                        const page = i + 1;
                        return (
                          <Button
