...
export default function EditPropertyPage({ params }: EditPropertyPageProps) {
  ...
  const { data: property, isLoading: isLoadingProperty } = tsr.agent.properties.get.useQuery({
    queryKey: ['agent-property', params.id],
    queryData: {
      params: { id: params.id }
    }
  })

  const { mutate: updateProperty, isPending } = tsr.agent.properties.update.useMutation({
    onSuccess: () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let addressLatitude = property?.body.addressLatitude || 0
    let addressLongitude = property?.body.addressLongitude || 0

    // If address changed, geocode it
    if (formData.address !== property?.body.address) {
      try {
        ...
        if (geocodeData.features && geocodeData.features.length > 0) {
          [addressLongitude, addressLatitude] = geocodeData.features[0].center
        } else {
          toast.error('Could not validate address. Please check the address and try again.')
          return
        }
      } catch (error) {
